...
from .augmentation_aug import MotionAugmentation
    ...
    def _get_random_video_background(self,files):
        if self.special_clip:
            if random.random() < 0.2:
                ...
                available_clips = [clip for clip in self.background_video_clips[:-len(self.frameToBgrDict)] if clip not in self.frameToBgrDict.items()]
        ...
        if not self.special_clip and clip in self.frameToBgrDict.items():
        ...
        for i in self.seq_sampler(self.seq_length):
                ...
                try:
                    ...
                    print(e,"\n\tclip_idx_base: {}\n\tframe_idx_t: {}\n\t frame_count: {}\n\t".format(clip_idx_base, frame_idx_t,frame_count))
                with Image.open(os.path.join(self.background_video_dir, clip_base, frame)) as base:
                    base = self._downsample_if_needed(base.convert('RGB'))
                bases.append(base)
