...
class MotionAugmentation:
    ...
    def __call__(self, fgrs, phas, bgrs, bases, special):
        
        # Motion Jitter
        fgrs, phas, bgrs = self._motion_jitter(fgrs, phas, bgrs )
        
        
        # # Foreground affine
        # if not special and random.random() < self.prob_fgr_affine:
        #     fgrs, phas = self._motion_jitter(fgrs, phas)

        # # Background affine
        # if not special and random.random() < self.prob_bgr_affine / 2:
        #     bgrs = self._motion_jitter(bgrs)
        # if random.random() < self.prob_bgr_affine / 2:
        #     # print("--------------Applying special fg/bg affine")
        #     fgrs, phas, bgrs = self._motion_jitter(fgrs, phas, bgrs)

        # # Still Affine
        # if self.static_affine:
        #     if special:
        #         # print("--------------Applying special still affine")
        #         fgrs, phas, bgrs = self._static_affine(fgrs, phas, bgrs, scale_ranges=(0.5, 1))
        #     else:
        ...
        bases = torch.stack([F.to_tensor(base) for base in bases])

        # Resize
        fgr_shape_h = fgrs[0][0].shape[0]
        fgr_shape_w = fgrs[0][0].shape[1]
        params = transforms.RandomResizedCrop.get_params(fgrs, scale=(1,1), ratio=self.aspect_ratio_range)
        print('BEFORE:',fgrs[0].shape,phas[0].shape,bgrs[0].shape)
        print('PARAMS:',*params)
        ...
        bgrs = torch.nn.functional.interpolate(bgrs,size=fgrs[0][0].shape, mode='bilinear',align_corners=False)
        bases = torch.nn.functional.interpolate(bases,size=fgrs[0][0].shape, mode='bilinear',align_corners=False)
        
        # bgrs = F.resized_crop(bgrs, *params, fgrs[0][0].shape, interpolation=F.InterpolationMode.BILINEAR)
        # bases = F.resized_crop(bases, *params,fgrs[0][0].shape, interpolation=F.InterpolationMode.BILINEAR)

        print('AFTER:',fgrs[0].shape,phas[0].shape,bgrs[0].shape,bases[0].shape)
        
        # # Horizontal flip
        # if special:
        #     if random.random() < self.prob_hflip:
        #         # print("--------------Applying special fg/bg hflip")
        #         fgrs = F.hflip(fgrs)
        #         phas = F.hflip(phas)
        #         bgrs = F.hflip(bgrs)
        ...
        #         # print("--------------Applying special fg/bg color jitter")
        ...
        # # Blur
        # if special:
        #     # print("--------------Applying special fg/bg blur")
    ...
    def _motion_jitter(self,*imgs, cycle=1.5):
        config = dict(degrees=(-10, 10), translate=(0.1, 0.1),
                  scale_ranges=(0.9, 1.1), shears=(-5, 5), img_size=imgs[0][0].size)
        ...
        T = len(imgs[0])
        easing = random_easing_fn()
        print("Easing function:",str(easing))
        print("Jittering angle from {:.2f} to {:.2f}".format(angleA, angleB))
        print("Jittering translation from ({:.2f}, {:.2f}) to ({:.2f}, {:.2f})".format(transXA, transYA, transXB, transYB))
        print("Jittering scale from {:.2f} to {:.2f}".format(scaleA, scaleB))
        print("Jittering shear from ({:.2f}, {:.2f}) to ({:.2f}, {:.2f})".format(shearXA, shearYA, shearXB, shearYB))
        ...
        if cycle == 1:
            for t in range(T):
                ...
                shearX = lerp(shearXA, shearXB, percentage)
                shearY = lerp(shearYA, shearYB, percentage)
                print("Frame {}:\n\trotation: {}\n\tscale: {}\n\ttranslation: ({}, {})\n\tshear: ({}, {})\n\tpercentage: {}".format(t+1, angle, scale, transX, transY, shearX, shearY, percentage))
            ...
            forward = int(round(T / cycle))
            for t in range(forward):
                percentage = easing(t/ (forward-1))
                angle = lerp(angleA, angleB, percentage)
                transX = lerp(transXA, transXB, percentage)
                transY = lerp(transYA, transYB, percentage)
                scale = lerp(scaleA, scaleB, percentage)
            ...
            for t in range(forward, T):
                percentage = easing((t-forward+1) / (T-1))
                # angle = angleB - lerp(angleA, angleB*(cycle-1), percentage)
                # transX = transXB - lerp(transXA, transXB*(cycle-1), percentage)
                # transY = transYB - lerp(transYA, transYB*(cycle-1), percentage)
                # scale = scaleB - lerp(scaleA, scaleB*(cycle-1), percentage)
                # shearX = shearXB - lerp(shearXA, shearXB*(cycle-1), percentage)
                # shearY = shearYB - lerp(shearYA, shearYB*(cycle-1), percentage)
                angle = lerp(angleB, angleA, percentage)
                transX = lerp(transXB, transXA, percentage)
                transY = lerp(transYB, transYA, percentage)
                scale = lerp(scaleB, scaleA, percentage)
                shearX = lerp(shearXB, shearXA, percentage)
                shearY = lerp(shearYB, shearYA, percentage)
                
                print("Frame {}:\n\trotation: {}\n\tscale: {}\n\ttranslation: ({}, {})\n\tshear: ({}, {})\n\tpercentage: {}".format(t+1, angle, scale, transX, transY, shearX, shearY, 1-percentage))
