...
# Stage 1
CUDA_VISIBLE_DEVICES=0,1 python train_aug.py \
    ...
    --epoch-start 0 \
    --epoch-end 20 \
    --distributed-port 12349 \
    --disable-validation \
    --num-workers 0

CUDA_VISIBLE_DEVICES=0,1 python train_aug.py \
    --model-variant yolov8 \
    --dataset videomatte \
    --resolution-lr 512 \
    --seq-length-lr 15 \
    --learning-rate-backbone 0.0001 \
    ...
    --learning-rate-decoder 0.0002 \
    --learning-rate-refiner 0 \
    --checkpoint checkpoint/yolov8n-seg.pt \
    --checkpoint-dir checkpoint/stage1 \
    --log-dir log/stage1 \
    --epoch-start 0 \
    --epoch-end 20 \
...
# Stage 2
CUDA_VISIBLE_DEVICES=2,3 python train_aug.py \
    --model-variant yolov8 \
    --dataset videomatte \
    --resolution-lr 512 \
    --seq-length-lr 50 \
...
# Stage 3
CUDA_VISIBLE_DEVICES=3 python train_aug.py \
    --model-variant mobilenetv3 \
    --dataset videomatte \
    --train-hr \
    --resolution-lr 512 \
...
CUDA_VISIBLE_DEVICES=2,3 python train_aug.py \
    ...
    --learning-rate-refiner 0.0002 \
    --checkpoint checkpoint/new_run_with_better_pha/stage3/epoch-22.pth \
...
#! Change here for augmentation
from dataset.videomatte_aug import (
    VideoMatteDataset,
...
#! Change here for augmentation
from dataset.augmentation_aug import (
    TrainFrameSampler,
    ValidFrameSampler
)
...
class Trainer:
    ...
    def init_model(self):
        ...
        if self.args.checkpoint:
            ...
            if self.args.model_variant != 'yolov8':
                self.log(self.model.load_state_dict(
                torch.load(self.args.checkpoint, map_location=f'cuda:{self.rank}')))
            else:
                import copy
                
                curr_model = copy.deepcopy(self.model.state_dict())
                yolo_state_dict = torch.load(self.args.checkpoint, map_location=f'cuda:{self.rank}')
                yolo_state_dict = copy.deepcopy(yolo_state_dict['model'].state_dict())
                yolo_backbone_dict = {'backbone.{}'.format(k): v for k, v in yolo_state_dict.items()}
                max_key_len = max(len(k) for k in self.model.state_dict())
                [print(f"{k.ljust(max_key_len)}\t{v.shape}") for k, v in self.model.state_dict().items()]
               
                self.log(self.model.load_state_dict(yolo_backbone_dict,strict=False))

                # max_key_len = max(len(k) for k in self.model.state_dict())
                # # [print(f"{k.ljust(max_key_len)}\t{v.shape}") for k, v in self.model.state_dict().items()]
                # # print('========================================')
                # # [print(f"{k.ljust(max_key_len)}\t{v.shape}") for k, v in yolo_backbone_dict.items()]
                
                # # print('Old model:',self.model.state_dict()['backbone.model.0.conv.weight'],self.model.state_dict()['backbone.model.0.conv.weight'].shape)

                # # print('Yolo model:',yolo_backbone_dict['backbone.model.0.conv.weight'],yolo_backbone_dict['backbone.model.0.conv.weight'].shape)
                # self.log(self.model.load_state_dict(yolo_backbone_dict,strict=False))
                # # print('New model:',self.model.state_dict()['backbone.model.0.conv.weight'],self.model.state_dict()['backbone.model.0.conv.weight'].shape)

                # # Check if YOLO weights are loaded properly
                # print('\nWeights Loaded?')
                # max_key_len = max(len(k) for k in self.model.state_dict())
                # print('Same keys:{}'.format(list(prev_model.keys()) == list(self.model.state_dict().keys())))
                # for key in prev_model.keys():
                #     if prev_model[key].cpu().numpy().all() == self.model.state_dict()[key].cpu().numpy().all():
                #         continue
                #     else:
                #         print(f"Values for {key.ljust(max_key_len)}:\tdifferent")

                # # [print(f"{k.ljust(max_key_len)}\t{v.shape}") for k, v in self.model.state_dict().items()]
                # # if yolo_loaded:
                # #     print("YOLO weights loaded properly.")
                # # else:
                # #     print("ERROR: YOLO weights not loaded properly.")
    ...
    def train_mat(self, true_fgr, true_pha, true_bgr, downsample_ratio, tag):
        ...
        if self.args.dataset == 'videomatte':
            print(true_fgr.shape,true_pha.shape,true_bgr.shape)
            if true_fgr.shape!=true_bgr.shape:
                print("FGR shape =/= BGR shape since {} = {}".format(true_fgr.shape,true_bgr.shape))
